...
module.exports.createInstance = function createInstance( opts ) {
    ...
    const jauntyMiddleware = async function jauntyMiddleware( request, _, next ) {

        if ( request.method.toLowerCase() === 'options' ||
            ( opts.ignoreAuthentication.has( request.originalUrl ) ) ) {
        ...
        if ( request.headers && request.headers.authorization ) {
            ...
            return next( new Errors.BadSchemeError( 'Request headers for authorization were not set.' ) );
        ...
        try {
            ...
            if ( resolution.isValid ) {
                ...
                request[ opts.attachments.request ] = ( resolution.__NOOP__ ) ? {
                    isNoop: true,
                    ...decodedToken
                } : resolution.payload || decodedToken.payload;
    ...
    jauntyMiddleware.unless = require( 'express-unless' );
    return jauntyMiddleware;

};

module.exports.Errors = Errors;
module.exports.createACL = require( './acl' );
